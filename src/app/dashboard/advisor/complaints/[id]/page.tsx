...
async function ComplaintDetailPage({ params }: ComplaintDetailPageProps) {
  const complaint = await getComplaintById(Number(params.id));

  if (!complaint) {
    redirect("/dashboard/advisor/complaints");
  }
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between space-x-2">
          ...
          <ButtonLink href={getDashboardLink("advisor", "/complaints")} icon={ArrowLeft} variant="outline">
            Back
          </ButtonLink>
        </CardTitle>
      </CardHeader>
